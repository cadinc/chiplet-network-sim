...
NodeInCGKN::NodeInCGKN(int n_dims, int vc_num, int buffer_size, Channel internal_channel,
  ...
  for (int i = 0; i < 2 * n_dims; i++) in_buffers_[i]->channel_ = internal_channel;
...
CGroupKN::CGroupKN(int k, int n_dims, int cgroup_radix, int vc_num, int buffer_size,
  ...
  for (int i = 0; i < num_chiplets_; i++)
    nodes_.push_back(
        new NodeInCGKN(n_dims, vc_num, buffer_size, internal_channel, external_channel));
}
...
//
// Mesh mode (torus_mode=false):
//   Only boundary nodes may serve as external ports. Selects
//   cgroup_radix_ nodes evenly spaced among all boundary nodes.
//   For n=2, k=4: 4*4-4=12 boundary nodes → picks all 12.
//
// Torus mode (torus_mode=true):
//   All nodes are topologically equivalent (every node has the
//   same degree). External nodes are selected uniformly from the
//   full node set using a stride-based approach.
std::vector<int> CGroupKN::select_external_nodes(bool torus_mode) const {
  if (torus_mode) {
    // All nodes are equivalent in a torus — no boundary concept.
    // Pick cgroup_radix_ nodes uniformly using center-aligned stride.
    std::vector<int> selected;
    selected.reserve(cgroup_radix_);
    double stride = (double)num_chiplets_ / cgroup_radix_;
    for (int i = 0; i < cgroup_radix_; i++) {
      int idx = (int)(i * stride + stride / 2.0);
      idx = std::min(idx, num_chiplets_ - 1);
      selected.push_back(idx);
    }
    return selected;
  }

  // Mesh mode: collect boundary nodes then stride-select.
  ...
  if (needed > total) {
    std::cerr << "[CGroupKN] ERROR: need " << needed << " external nodes but only " << total
...
void CGroupKN::set_chip(System* dragonfly, int cgroup_id) {
  ...
  // ---- Wire mesh connections (interior hops only) ----
  // Connects each node to its positive neighbor in each dimension.
  // Works for both mesh and torus; torus wrap-arounds are added below.
  ...
  // ---- Torus wrap-around connections ----
  // For each dimension d, connect the "last-in-d" face to the "first-in-d" face.
  // This transforms the k-ary n-cube mesh into a k-ary n-cube torus.
  // Boundary nodes in dimension d now have full degree (no missing edge).
  if (df->use_torus_) {
    for (int d = 0; d < n_dims_; d++) {
      for (int node_id = 0; node_id < num_chiplets_; node_id++) {
        NodeInCGKN* node = get_node(node_id);
        if (node->coords_[d] == k_ - 1) {
          // This node is on the "last" face in dim d.
          // Connect its pos(d) port to the corresponding "first" face node's neg(d) port.
          std::vector<int> nb_coords = node->coords_;
          nb_coords[d] = 0;
          int nb_id = coords_to_id(nb_coords);
          NodeInCGKN* nb = get_node(nb_id);
          node->link_nodes_[node->port_pos(d)] = NodeID(nb_id, cgroup_id);
          node->link_buffers_[node->port_pos(d)] = nb->in_buffers_[nb->port_neg(d)];
          nb->link_nodes_[nb->port_neg(d)] = NodeID(node_id, cgroup_id);
          nb->link_buffers_[nb->port_neg(d)] = node->in_buffers_[node->port_pos(d)];
        }
      }
  ...
  external_nodes_ = select_external_nodes(df->use_torus_);
  ...
  if (cgroup_id == 0) {
    std::string mode = df->use_torus_ ? "torus" : "mesh";
    std::cout << "[CGroupKN:" << mode << "] k=" << k_ << " n=" << n_dims_
              << " chiplets=" << num_chiplets_ << " ext_nodes=" << external_nodes_.size()
              << std::endl;
  }
}
...
DragonflyChipletKN::DragonflyChipletKN() : num_cgroup_(num_chips_), cgroups_(chips_) {
  ...
  // cgroup_radix (external ports per C-group):
  //   n=2 mesh or torus: 4k-4 (same formula — the torus has the same external
  //     port count, different internal topology)
  //   n>=3 mesh: 4k  (subset of boundary nodes)
  //   n>=3 torus: 4k (uniform subset of all nodes, no boundary concept)
  if (n_dims_ == 2) {
    cgroup_radix_ = 4 * k_node_in_CG_ - 4;
    std::cout << "[DragonflyChipletKN] n=2" << (use_torus_ ? " torus" : " mesh")
              << ": cgroup_radix=4k-4=" << cgroup_radix_ << std::endl;
  } else {
    cgroup_radix_ = 4 * k_node_in_CG_;
  }

  // Validate enough nodes for external port assignment.
  if (!use_torus_) {
    int interior = 1;
    ...
    if (boundary < cgroup_radix_) {
      std::cerr << "[DragonflyChipletKN] ERROR: Only " << boundary << " boundary nodes but "
                << cgroup_radix_ << " external ports needed." << std::endl;
      exit(1);
    }
  } else {
    // Torus: all nodes available; only constraint is cgroup_radix <= num_chiplets
    if (cgroup_radix_ > num_chiplets_per_cg_) {
      std::cerr << "[DragonflyChipletKN] ERROR: cgroup_radix=" << cgroup_radix_
                << " > num_chiplets=" << num_chiplets_per_cg_ << std::endl;
      exit(1);
    }
  }
  ...
  if (single_cgroup_) {
    ...
    std::cout << "[DragonflyChipletKN] Single C-group mode"
              << "  k=" << k_node_in_CG_ << "  n=" << n_dims_ << (use_torus_ ? "  torus" : "  mesh")
              << "  chiplets=" << num_chiplets_per_cg_ << "  ext_ports=" << cgroup_radix_
              << "  num_cores=" << num_cores_ << std::endl;
    cgroups_.push_back(new CGroupKN(k_node_in_CG_, n_dims_, cgroup_radix_, param->vc_number,
  ...
  if (max_wgroups_ > 0 && num_wgroup_ > max_wgroups_) {
    std::cout << "[DragonflyChipletKN] Capping num_wgroup from " << num_wgroup_ << " to "
  ...
  std::cout << "[DragonflyChipletKN]" << (use_torus_ ? " torus" : " mesh")
            << "  k=" << k_node_in_CG_ << "  n=" << n_dims_
            << "  chiplets_per_cg=" << num_chiplets_per_cg_ << "  ports_per_node=" << (2 * n_dims_)
            << "+1"
            << "  ext_ports_per_cg=" << cgroup_radix_ << "  l=" << l_ports_per_cg_
...
void DragonflyChipletKN::read_config() {
  ...
  use_torus_ = param->params_ptree.get<bool>("Network.use_torus", false);
  if (n_dims_ < 2) {
    std::cerr << "ERROR: n_dims >= 2 required\n";
    exit(1);
  }
  if (k_node_in_CG_ < 2) {
    std::cerr << "ERROR: k_node >= 2 required\n";
    exit(1);
  }
}
...
//
// Mesh mode: route greedily in dimension-order (X then Y then ...).
// Torus mode: use shortest-path in each dimension, accounting for
//   wrap-around. The actual link traversal follows whatever buffer
//   is wired to port_neg/port_pos — the wrap-around link IS wired
//   to port_pos of the last node in each dimension, so routing
//   toward the "closer" direction automatically uses the right link.
//
// Tie-breaking at the midpoint (k even, delta = k/2): prefer positive
...
void DragonflyChipletKN::XD_routing(Packet& s, NodeID dest, int vcb) const {
  ...
  for (int d = 0; d < n_dims_; d++) {
    ...
    if (use_torus_) {
      // Choose the shorter direction around the torus ring of size k.
      // 2*delta > k  means going negative (wrap-around) is shorter.
      // 2*delta < -k means going positive (wrap-around) is shorter.
      if (2 * delta > k_node_in_CG_) delta -= k_node_in_CG_;
      if (2 * delta < -k_node_in_CG_) delta += k_node_in_CG_;
    }
    if (delta < 0) {
    ...
    // delta == 0: already aligned in this dimension, try next
  }
}
